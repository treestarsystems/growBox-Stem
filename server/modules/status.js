...
var collectedDataLocal = {
 ...
 "caseTemperature": internalCaseTemperature.data[0],
 "hostname": os.hostname(),
 "uptimeMin": (os.uptime()/60).toFixed(),
...
//    collectedDataLocalDiskData[`disk${index}`] = {
    let data = {
     "name": `disk${index}`,
     "path": path,
    ...
    collectedDataLocalDiskData.push(data);
   }
  })
 });
...
function collectSystemStatusInterfaces(ints) {
 ...
 for (const [key, value] of Object.entries(ints)) {
  var i = 0;
  if (key != 'lo') {
   collectedDataLocalNetwork[key] = {
   "interfaceMac": value[0]['mac']
   };
...
async function sendCollectedData() {
 let data = await aggregateSystemStatus();
